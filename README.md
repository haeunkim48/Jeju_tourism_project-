...
PPT : SangMingyu(https://github.com/SangMingyu), HaJeong-K(https://github.com/HaJeong-K)
...
API 사용을 위해 여러 관광지별로 축소하는 것으로 주제 변경.
...
3번의 주제를 그대로 이끌고 가되 머신러닝을 사용해보는 것을 추가로 생각함.
비슷한 주제에 대한 논문 수집 및 사용된 분석 기법 참고.
3번의 주제를 그대로 두고, 종속변수를 확정함.
...
최종 주제로 변경 = 코로나를 제외 시킨 제주 여행에 대한 관광객 수 현황 및 인기 관광지 정보 및 현 트렌드 제공.
...
인기 관광지 : 셀레니움을 사용해 5회 이상 언급된 6개 관광지 대상으로 각각 리뷰 110개씩 추출(trip advisor), konlpy를 통해 형태소 분리 후 stopwords 작성하여 리뷰 데이터 텍스트 마이닝 중, 크롤링 및 텍스트 마이닝을 진행한 모든 제주 관광지 지도시각화 완료.
